...
3. 左侧上方会形成连续对话记录，底部的同一个输入框用于首次需求和后续修改；每条成功回复附带该版本的 Product Brief、Technical Plan 和 Validation 入口，点击后在右侧查看。右侧也可操作预览，或在 Code 标签中切换虚拟的 HTML、CSS 和 JavaScript 文件。
...
- 对话式两栏工作台：左侧上方是需求与版本回复记录、底部是统一输入框；结构化产物作为版本回复的紧凑附件，点击后在右侧“产物”面板查看，避免常驻卡片挤占对话空间。
...
- 三个一键示例、生成取消、版本产物入口、代码、事件日志、验证面板和 HTML 下载。
